...
public interface PublicationRepository extends JpaRepository<Publication, Long>, JpaSpecificationExecutor<Publication> {
    ...
    // Publicaciones de un usuario en un estado (ej: las activas para su perfil publico)
    List<Publication> findBySellerIdAndStateOrderByCreatedAtDesc(Long sellerId, PublicationState state);
}
